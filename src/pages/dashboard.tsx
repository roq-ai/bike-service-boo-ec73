...
import { useRoqPlatformUserProfiles, useServiceCount, useMechanicCount, useServiceManagerCount } from 'lib/roq';

import { MechanicListPage as ShowCaseList } from 'pages/mechanics';
...
function DashboardPage() {
  ...
  const mechanicStat = useMechanicCount();

  const serviceManagerStat = useServiceManagerCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Mechanics',
      result: mechanicStat?.data,
      isLoading: mechanicStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Service Managers',
      result: serviceManagerStat?.data,
      isLoading: serviceManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`mechanics`} passHref legacyBehavior>
                  <Button
